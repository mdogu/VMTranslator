    ...
    func execute() {
        while let line = inputFile.readLine() {
            ...
            case "add", "sub", "neg", "eq", "gt", "lt", "and", "or", "not":
                ...
                codeWriter?.write(command: .arithmeticLogical(al))
            case "push", "pop":
                guard parts.count == 3,
                ...
                switch command {
                case "push":
                    codeWriter?.write(command: .memoryAccess(.push(segment, index)))
                case "pop":
                    codeWriter?.write(command: .memoryAccess(.pop(segment, index)))
                default:
                    break
                }
